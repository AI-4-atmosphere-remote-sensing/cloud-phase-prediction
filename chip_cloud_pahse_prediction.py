...
def train_model(train_dat, valid_dat, model, n_epochs, lambda_, lambda_l2, device):
    ...
    ''' SERAJ >> commenting out the following part (optimizer = Adam ...) to handle single GPU while the code is adopted to multi-GPU. 
    when using DataParallel, the model is wrapped in a module attribute, but if you're using only one GPU (or no GPUs), the model isn't 
    wrapped in DataParallel and doesn't have a module attribute. To Fix: we can modify the way you access the model parameters by checking 
    if model is wrapped with DataParallel before trying to access module. If it's not wrapped, access the model parameters directly.
    '''
    ...
    if isinstance(model, torch.nn.DataParallel):
        # If model is wrapped with DataParallel, use model.module
        optimizer = Adam([
            {'params': model.module.ddm.parameters(), 'lr': 0.1},
            {'params': model.module.feature.parameters()},
            {'params': model.module.fc.parameters(), 'lr': 0.05}
        ], lr=0.05)
    else:
        # If the model is not wrapped with DataParallel, access parameters directly
        optimizer = Adam([
            {'params': model.ddm.parameters(), 'lr': 0.1}, #.001
            {'params': model.feature.parameters()},
            {'params': model.fc.parameters(), 'lr': 0.05} # 0.000005
        ], lr=0.05)
...
## Added these lines << Seraj
  ...
  lambda_ = 0.001
  lambda_l2 = 0.05
  BATCH_SIZE = 20480 # < for 44 ##2048 < for > was originally 440
...
# time capture ends. added by Seraj
  end_time = time.time()  # Record the end time

  # Calculate total time taken (in minutes and seconds)
  ...
  print(f"Total time taken: {int(minutes)} minutes and {int(seconds)} seconds using {args.num_gpus} GPUs.")
