...
# === Accumulators ===
liquid_pred = np.zeros(grid_shape, dtype=int)
cloud_pred = np.zeros(grid_shape, dtype=int)
liquid_gt = np.zeros(grid_shape, dtype=int)
...
for day in days:
    ...
    for i, fname in enumerate(os.listdir(v03_folder), 1):
        ...
        # Prediction
        pred_liquid = pred_valid == 1
        pred_cloud = (pred_valid == 1) | (pred_valid == 2)
        np.add.at(liquid_pred, (glat[pred_liquid], glon[pred_liquid]), 1)
        np.add.at(cloud_pred, (glat[pred_cloud], glon[pred_cloud]), 1)

        # Ground Truth
        gt_liquid = gt_valid == 1
        gt_cloud = (gt_valid == 1) | (gt_valid == 2)
        np.add.at(liquid_gt, (glat[gt_liquid], glon[gt_liquid]), 1)
        np.add.at(cloud_gt, (glat[gt_cloud], glon[gt_cloud]), 1)

# === Compute liquid cloud fractions ===
liquid_fraction_pred = np.full(grid_shape, np.nan)
liquid_fraction_gt = np.full(grid_shape, np.nan)

valid_pred = cloud_pred > 0
valid_gt = cloud_gt > 0

liquid_fraction_pred[valid_pred] = liquid_pred[valid_pred] / cloud_pred[valid_pred]
liquid_fraction_gt[valid_gt] = liquid_gt[valid_gt] / cloud_gt[valid_gt]

# === Save ===
np.save('/umbc/rs/nasa-access/xin/cloud-phase-prediction/data/liquid_fraction2017_31days.npy', liquid_fraction_pred)
np.save('/umbc/rs/nasa-access/xin/cloud-phase-prediction/data/liquid_fraction_GT2017_31days.npy', liquid_fraction_gt)

print("Liquid cloud fraction processing complete.")
